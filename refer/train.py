...
add_arg('gpus',             str,    '0',                      '训练使用的GPU序号，使用英文逗号,隔开，如：0,1') #参数名 类型 默认值 描述
...
args = parser.parse_args() #可通过args.XXX来调用,如args.batch_size
...
def train():
    device_ids = [int(i) for i in args.gpus.split(',')] #逗号隔开
    # 数据输入的形状
    ...
    if len(args.gpus.split(',')) > 1: #GPU并行训练
    ...
    optimizer = torch.optim.SGD([{'params': model.parameters()}, {'params': metric_fc.parameters()}],
                                lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay) #SGD optimizer:梯度下降
    # 获取学习率衰减函数
    scheduler = StepLR(optimizer, step_size=args.lr_step, gamma=0.1, verbose=True) #α
    ...
    for epoch_id in range(last_epoch, args.num_epoch):
        for batch_id, data in enumerate(train_loader):
            ...
            optimizer.zero_grad() #清除梯度,通常每个batch前用
            loss.backward()
            optimizer.step()

            if batch_id % 100 == 0: #每100次打印
                output = output.data.cpu().numpy()
                output = np.argmax(output, axis=1)
                label = label.data.cpu().numpy()
                acc = np.mean((output == label).astype(int))
