...
def get_data_list(infodata_path, list_path, zhvoice_path): #json文件路径 测试集/训练集路径 MP3文件路径
    """
    infodata_path: the path to a JSON file that contains metadata about the audio files in the dataset (e.g. speaker information, duration)
    list_path: the path to a directory where the script will save two text files (train_list.txt and test_list.txt) that contain the file paths and corresponding labels for the training and testing sets of the dataset
    zhvoice_path: the path to a directory that contains the audio files in the dataset (in MP3 format).
    """
    with open(infodata_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()# 读取每一行，存在列表中

    f_train = open(os.path.join(list_path, 'train_list.txt'), 'w') # 要写的文件
    f_test = open(os.path.join(list_path, 'test_list.txt'), 'w')# 要写的文件
